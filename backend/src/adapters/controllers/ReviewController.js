...
module.exports = class ReviewController {
    static async create(req, res) {
        ...
        try {
            const review = await createReview.execute({ rating, comment, petId, authorId})
    ...
    static async getAll(req, res) {
        ...
        try {
            ...
            return res.status(500).json({message: 'Erro interno so listar avaliações'})
        }
    }
    static async getAll(req, res){
        const reviewRepository = new ReviewMongoRepository()
    ...
    static async getByPet(req, res){
        ...
        try{
            ...
            return res.status(500).json({message: 'Erro interno ao listar avaliações do pet'})
        }
    }
}